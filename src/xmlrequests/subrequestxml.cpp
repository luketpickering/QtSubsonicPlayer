...
SubRequestXML::SubRequestXML(QString* _host, int* _port, QString* _usr, QString* _pss)
{
	host = _host;
	usr = _usr;
	pss = _pss;
...
{
	QString authDetails_ = "Basic " + (*usr + ":" + *pss).toLocal8Bit().toBase64();
...
{
	printf("Recieved response from: " + netReq->url()
		.host().toLocal8Bit() + "\n");
	if(netReply->error() == QNetworkReply::NoError )
	{
		...
		printf("Http error code:%i -" + netReply->errorString().toLocal8Bit(), netReply->error());
}

bool SubRequestXML::isHTTPRedirect()
{
	int httpSC;
	httpSC = netReply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
	if(httpSC == 302)
	{
		netReply->close();
		QUrl redirect_ = netReply->attribute(QNetworkRequest::RedirectionTargetAttribute).toUrl();
		delete netReply;

		printf(netReq->url().host().toLocal8Bit() + " requests redirect to: " + redirect_.host().toLocal8Bit() + "\n");
		*host = redirect_.host();
		*port = redirect_.port();
...
int SubRequestXML::isHealthySubResp()
{
	...
	{
        QDomElement responseNode = respXML->namedItem("subsonic-response").toElement();

		if(!responseNode.isNull())
		{
			if(responseNode.attribute("status",QString("NULL")).toLocal8Bit() == "failed")
			{
				bool parsedEC;
				int eC = 0;
